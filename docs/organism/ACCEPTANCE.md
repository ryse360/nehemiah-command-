...
| Production orb connection (step 3) | DONE | The authenticated Founder dashboard now renders the **approved organism**, not the old CSS artifact. One reusable presentation component (`OrganismEngine`) is shared: `/lab/organism` still drives it with synthetic fixtures (Leva + the state machine), while the dashboard drives it through `NehemiahOrb`, whose only prop is a sanitized `OrbStateDTO`. Salience is derived by the pure, server-capable `computeOrbState()` (`orb-state.ts`) into derived-only values — operating state, node count, priority bands, recency weight, intensity, convergence, open-loop/settled counts — and mapped to render parameters by the pure `orbParametersFromState()` (`orb-parameters.ts`), which starts from the SAME `resolveStateParameters()` the lab uses and applies only small bounded modulations, so **the approved design is unchanged** (a mapper test asserts camera/scale/core/breath/rotation/lighting are taken verbatim). **The client boundary is enforced two ways:** contract tests prove no Founder content, timestamp, id, or disposition survives into the serialized DTO, and a compliance check forbids `NehemiahOrb` from importing any raw-Founder-data module. The old `living-organism.tsx` was deleted so the superseded design can't be re-wired. **Honest divergence from the brief:** the brief said "the server, not the browser, calculates salience"; today Founder memory is client-held (localStorage + cloud sync) and the operating state is client-ephemeral UI state, so `computeOrbState()` runs in the browser. It is written as a pure module with no DOM/React deps, positioned to move server-side unchanged once memory becomes server-authoritative. The enforced, testable guarantee — raw Founder data never reaches the orb — holds regardless of where the pure function runs. |
