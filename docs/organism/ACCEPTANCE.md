...
| Not-yet-installed platform nodes (need a non-npm path or a decision) | OPEN | shadcn/ui needs Tailwind (not in project — restructures styling; deferred). Graphiti is Python (separate backend service, not an npm dep). Context7 + Playwright MCP are MCP servers (config-level, not project deps). Wiring AI SDK + memory to real Founder data is a separate security-gated step (keys/PII/production surface per MASTER_AI_HANDOFF). |
