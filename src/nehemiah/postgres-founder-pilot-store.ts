  ...
  async append(founderId: string, session: FounderPilotSession): Promise<FounderPilotSession> {
    const rows = await this.sql<{ session_id: string }[]>`
      insert into founder_pilot_sessions (
        ...
        proof_captured, decision_useful, five_second_gate, critical_issues, high_issues, accepted_high_issues, notes
      ) values (
        ...
        ${session.criticalIssues}, ${session.highIssues}, ${session.acceptedHighIssues}, ${session.notes}
      ) on conflict (founder_id, session_id) do nothing
      returning session_id
    `;
    if (rows.length === 0) throw new Error('Pilot session already exists.');
    return session;
  }
  ...
  async list(founderId: string): Promise<FounderPilotSession[]> {
    const rows = await this.sql<any[]>`
      select session_id, started_at, ended_at, command, completed_journey, proof_captured,
             decision_useful, five_second_gate, critical_issues, high_issues, accepted_high_issues, notes
    ...
    return rows.map((row) => ({
      ...
      criticalIssues: row.critical_issues, highIssues: row.high_issues, acceptedHighIssues: row.accepted_high_issues, notes: row.notes,
    }));
  }
}
